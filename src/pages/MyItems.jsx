...
export default function MyItems() {
  ...
  return (
    <div className="min-h-screen bg-sky-100 dark:bg-gray-900/80 p-4 md:p-10">
      <h2 className="text-2xl md:text-3xl font-bold text-sky-700 dark:text-white mb-6 text-center">My Items</h2>
      {myFoods.length === 0 ? (
        <p className="text-center text-gray-700 dark:text-gray-300">No items added yet.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="table w-full bg-white/30 dark:bg-gray-900/30 backdrop-blur-md shadow-2xl rounded-2xl">
            <thead>
              <tr>
                <th>Image</th>
                <th>Title</th>
            ...
            <tbody>
              {myFoods.map((food) => (
                <tr key={food._id}>
                  <td><img src={food.imageUrl || "/src/assets/default-food.jpg"} className="h-16 w-16 object-cover rounded" /></td>
                  <td>{food.title}</td>
                  <td>{food.category}</td>
                  <td>{food.quantity}</td>
